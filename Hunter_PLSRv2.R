...
#viu que a parte do espectro na UV mostrou muito variabilidade
#para tirar este parte do espectro precisa criar indice que mostra 
#onde que tem valores de comprimento de onda acima de 426
ind = which(wavelengths>426)
matplot(wavelengths[ind], t(VN_cal[2:11,ind]), type='l')

#tira os dados da regiao ultravioleta dos espectros
VN_cal = VN_cal[,ind]
VN_val = VN_val[,ind]
...
plot(RMSEP(pls_C), ylab='RMSEP', xlab = 'ncomp', type='l',ylim=c(8,60))
...
pls_Cv =  predict(pls_C, ncomp = 5, newdata = Valdeir_Test)

#outros plots de validacao
plot(pls_C,ncomp=10, asp=1, line=T) 
plot(pls_C, plottype = "scores", comps = 1:5) #scores de cada componente
plot(pls_C, "loadings", comps = 3:4) #ver os loadings
abline(h = 0)
